...
public class JanelaManterLugar extends JanelaCrudModelo {
	private JTextField textFieldNome;
	private JComboBox comboBoxAtivo;
	private JButton btnInativar;

	public JanelaManterLugar(IControladorCRUD contr) {
		super(contr);
		...
		btnInativar.setIcon(new ImageIcon(JanelaManterLugar.class.getResource("/icons/No.png")));
		getToolBar().add(btnInativar);
		// TODO Auto-generated constructor stub
	}

	protected void actionInativar() {
		Lugar tab = (Lugar)this.getObjetoLinhaSelecionada();
		if(tab == null){
			JOptionPane.showMessageDialog(null, "Erro: Selecione uma linha antes de Inativar!");
			return;
		}
		ControladorManterLugar ctl = (ControladorManterLugar)getControlador();
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					JanelaManterLugar frame = new JanelaManterLugar(new ControladorManterLugar());
